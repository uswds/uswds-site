...
{: .usa-table }
Setting | Default values - Version 2.x | Default values - Version 3.x | Description
--- | --- | --- | ---
...
`paths.src.components` | Not available | `"./node_modules/@uswds/uswds/dist/components"` | Source location of the USWDS web component files
...
`paths.dist.components` | `"./assets/uswds/components"` (unused) | `"./assets/uswds/components"` | Project destination for web component files
...
`settings.compile.sassDeprecationWarnings` | `false` | `false` | Show USWDS Sass deprecation warnings. When set to `true`, Sass will output deprecation <br/>warnings for USWDS code in the terminal during compilation. Deprecation warnings for non-USWDS <br/>Sass will output even when this value is set to `false`.
...
If you use a USWDS web component such as `<usa-banner>`, configure its destination separately from `paths.dist.js`. For example:

```js
uswds.paths.dist.js = './assets/js';
uswds.paths.dist.components = './assets/components';
exports.copyAssets = uswds.copyAssets;
```

Run `npx gulp copyAssets` after changing these paths, then update the web component module URL in your HTML to match the files in the new destination. Changing a destination copies files; it does not rewrite script URLs in your templates. Keep the copied component directory structure intact. The `paths.dist.js` setting controls the traditional JavaScript bundle, while `paths.dist.components` controls web component files.

Web component copying requires a version of `@uswds/compile` that exports `copyWebComponents`. USWDS 2 does not provide a web component source directory. For USWDS 3 packages without that directory, the copy task skips it.
...
`copyAssets` | Copies all static assets: `copyFonts` + `copyImages` + `copyJS` + `copyWebComponents`
...
`copyWebComponents` | Copy USWDS web component files to `paths.dist.components`, when the source directory exists
