...
const PAGES = ["/", "/page-templates/landing/", "/page-templates/docs/"];
...
Promise.all([runServer(), getChrome()])
  .then(([server, chrome]) => {
      ...
      (client) => {
        ...
        const loadNextPage = () => {
          if (pagesLeft.length === 0) {
            console.log(
              `Finished visiting ${PAGES.length} pages with no errors.`
            );
            terminate(0);
          } else {
        ...
        Promise.all([Page.enable(), Network.enable()]).then(() => {
            ...
            Runtime.evaluate({
              ...
              if (details.result.type === "string") {
                ...
                if (viols.length === 0) {
                  ...
                  console.log(
                    chalk.red(`Found ${viols.length} aXe violations.`)
                  );
                  console.log(viols);
                  console.log(
                    chalk.cyan(
